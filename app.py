...
def parse_data(file, file_type):
    ...
    # Ищем строку, где есть слово 'Контроль' или максимальное количество строковых значений
    header_idx = 0
    ...
    # Обрезаем все, что было до заголовка, и первую колонку (обычно это названия рыб/повторностей)
    ...
    # Если первый столбец содержит текст (названия повторностей), удаляем его или делаем индексом
    first_col_is_str = df_clean.iloc[:, 0].astype(str).str.contains('[a-zA-Zа-яА-Я]').any()
    if first_col_is_str:
        df_clean = df_clean.iloc[:, 1:]
    
    # Переводим всё в числа, ошибки (текст) превращаем в NaN
    for col in df_clean.columns:
        df_clean[col] = pd.to_numeric(df_clean[col], errors='coerce')
        
    df_clean = df_clean.dropna(how='all', axis=0) # удаляем строки где одни NaN (остатки шапки)
    
    # Очищаем названия колонок от NaN
